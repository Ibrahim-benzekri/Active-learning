# Active Learning (Diversity) | Cluster-based Sampling (K-Means on hidden embeddings)
...
def make_balanced_loader(X_cpu: torch.Tensor, y_cpu: torch.Tensor, batch_size=256):
    ...
    (Selection strategy is K-Means diversity; sampler is only for training stability.)
    """
    y_np = y_cpu.numpy()
...
# -----------------------------
# Hidden extraction + KMeans selection
# -----------------------------
@torch.no_grad()
...
def kmeans_select_from_pool(model, X_pool_cpu, pool_indices, k, device, rep_seed):
    """
    K-Means on hidden embeddings of unlabeled pool.
    Select 1 point closest to each centroid => k points (diverse).
    """
    if k <= 0 or X_pool_cpu.shape[0] == 0:
        return np.array([], dtype=int)

    # If pool smaller than k
    if X_pool_cpu.shape[0] <= k:
        return np.array(pool_indices, dtype=int)

    # 1) embeddings
    emb = hidden_embeddings(model, X_pool_cpu, device=device)  # [N,128]

    # 2) KMeans with k clusters
    km = KMeans(n_clusters=k, random_state=rep_seed, n_init=10)
    labels = km.fit_predict(emb)
    centers = km.cluster_centers_  # [k,128]

    # 3) pick closest point to each centroid
    selected_local = []
    for c in range(k):
        ...
        best_local = idxs[int(np.argmin(d2))]
        selected_local.append(best_local)

    selected_local = np.array(selected_local, dtype=int)
    selected_original = np.array(pool_indices, dtype=int)[selected_local]

    # In rare cases <k (empty cluster), fill randomly among remaining
    if selected_original.shape[0] < k:
        remaining = np.setdiff1d(np.array(pool_indices, dtype=int), selected_original, assume_unique=False)
        need = k - selected_original.shape[0]
        rng = np.random.default_rng(rep_seed)
        if remaining.shape[0] > 0:
            extra = rng.choice(remaining, size=min(need, remaining.shape[0]), replace=False)
            selected_original = np.concatenate([selected_original, extra])
...
def main():
    ...
    logs_path = os.path.join("artifacts", "al_kmeans_sampling_retrain_logs.txt")

    # reset logs
    with open(logs_path, "w", encoding="utf-8") as f:
        f.write("")

    log_txt(logs_path, "===== Active Learning (Diversity) | Cluster-based K-Means (hidden embeddings) =====")
    ...
    log_txt(logs_path, "selection: K-Means on hidden embeddings of unlabeled pool; select nearest-to-centroid points")
    log_txt(logs_path, f"training: epochs={epochs} lr={lr} batch_size={batch_size} sampler=WeightedRandomSampler")
    log_txt(logs_path, "evaluation: SMS test only (fixed split=0.2)")
    log_txt(logs_path, "")
    ...
    for rep in range(1, repeats + 1):
        ...
        for it in range(1, iters + 1):
            ...
            # 1) Scoring model train (EMAIL + current labeled)
            ...
            # 2) KMeans selection on unlabeled hidden embeddings
            if need > 0 and len(unlabeled_set) > 0:
                ...
                selected = kmeans_select_from_pool(
                    model=scoring_model,
                ...
                log_txt(logs_path, f"  selection(kmeans): need={need}, selected={len(selected)}, labeled_now={len(labeled_set)}")
            else:
                log_txt(logs_path, f"  selection(kmeans): need={need}, selected=0, labeled_now={len(labeled_set)}")

            # 3) Final retrain from scratch (EMAIL + labeled after selection)
            if len(labeled_set) == 0:
