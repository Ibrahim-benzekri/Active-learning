# Active Learning (Entropy Sampling) | Retrain from scratch each iteration
# 5 iterations x 3 repeats | logs to TXT
...
def make_balanced_loader(X_cpu: torch.Tensor, y_cpu: torch.Tensor, batch_size=256):
    ...
    Selection strategy is entropy sampling; sampler is only for training stability.
    """
    y_np = y_cpu.numpy()
...
# -----------------------------
@torch.no_grad()
def entropy_scores(model, X_pool_device):
    """
    Entropy: H(p) = -sum_i p_i log(p_i)
    Higher entropy => more uncertain => select those with largest entropy.
    """
    model.eval()
    ...
    # numerical stability
    eps = 1e-12
    ent = -(probs * torch.log(probs + eps)).sum(dim=1)  # [N]
    return ent.detach().cpu().numpy()

def select_by_entropy(model, X_pool_cpu, pool_indices, k, device, rep_seed):
    """
    ...
    ent = entropy_scores(model, X_pool_device)

    # tie-breaking tiny noise
    rng = np.random.default_rng(rep_seed)
    ent = ent + rng.normal(0, 1e-12, size=ent.shape)

    order = np.argsort(-ent)[:k]  # descending entropy
...
# Selection uses entropy sampling from unlabeled SMS pool
# -----------------------------
def main():
    # --- Config ---
    repeats = 3
    ...
    logs_path = os.path.join("artifacts", "al_entropy_sampling_retrain_logs.txt")

    # reset log file
    with open(logs_path, "w", encoding="utf-8") as f:
        f.write("")

    log_txt(logs_path, "===== Active Learning (Entropy Sampling) | Retrain from scratch each iteration =====")
    ...
    log_txt(logs_path, f"train: EMAIL(train) + labeled_SMS_subset | selection: Entropy (max entropy)")
    ...
    # Target labeled sizes per iteration: 20%,40%,...,100%
    ...
    for rep in range(1, repeats + 1):
        ...
        for it in range(1, iters + 1):
            ...
            # 1) Train scoring model (EMAIL + current labeled SMS)
            ...
            # 2) Entropy selection from unlabeled pool
            ...
            # 3) Final retrain-from-scratch (EMAIL + labeled after selection) + evaluate
            if len(labeled_set) == 0:
